...
def calculate_total_quantity(txt_file, excel_file):
    ...
    for sheet_name in sub_table_names:
        ...
        # 计算子表中所有元素的平均值
        if not df.empty:
            average = df.values.sum()
            sub_table_averages.append(average)
        else:
            sub_table_averages.append(0)  # 如果子表为空，则平均值为 0

    print(f"The sheet number is {len(sub_table_averages)}\n")
    print(f"The repeat data number is {len(txt_data)}\n")

    # 确保 txt 文件中的数据量与 Excel 文件中的子表数量一致
    if len(txt_data) != len(sub_table_averages):
        raise ValueError("txt 文件中的数据量与 Excel 文件中的子表数量不一致")

    # 计算总数量
    total_quantity = 0
    for i in range(len(txt_data)):
        total_quantity += txt_data[i] * sub_table_averages[i]
...
if __name__ == '__main__':
    ...
    parser.add_argument('-net', type=str, default='xception', help='net type')
    args = parser.parse_args()

    model_name = args.net

    txt_file = f'compute_repeat_element_{model_name}.txt'
    excel_file = f'delay_matrix_{model_name}.xlsx'  # 请替换成您的 Excel 文件名
