...
class QuantizedConvLayer(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, stride=1, padding=0, groups=1, bias=False, process_fn=None, mre_fn=None):
        ...
        self.relu = nn.ReLU()
        self.process_fn = process_fn  # 实例化额外处理函数(ReLU/BN等)
        self.mre_fn = mre_fn  # 实例化误差函数(引入线性度误差)
        # 5-bit 量化权重
...
class QuantizedLinearLayer(nn.Module):
    def __init__(self, in_features, out_features, bias=False, process_fn=None, mre_fn=None):
        """
